...
data = pd.read_csv('Daily Plan/adjust weekday.csv')
...
for pid in unique_pids:
    ...
    for index, row in df_subset.iterrows():
        ...
        for col in ['firstbath', 'secondbath', 'breakfast', 'lunch', 'dinner', 'washclothes', 'clean', 'footbath']:
            ...
            activity.append([start_time, period])

        for i, act in enumerate(activity):
            activity[i] = calculate_finish(act[0], act[1], row['weekday_deptime'], row['weekday_endtime'])

        adjusted_activities = adjust_activity_schedule(activity, row['weekday_deptime'], row['weekday_endtime'])
...
new_data.to_csv('Daily Plan/adjust weekday new.csv', index=False)