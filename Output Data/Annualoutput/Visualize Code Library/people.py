...
num_bins = 5
# 几何划分
percentiles = np.percentile(all_people_Peomean, np.linspace(0, 100, num_bins + 1))
...
for i, (shp_file, gdf) in enumerate(zip(shp_files, gdf_list)):
    ...
    #cut进行几何划分
    gdf['bin_Pop'] = pd.cut(gdf[Peomean_column_name], bins=percentiles, labels=range(num_bins))

    gdf.plot(column='bin_Pop', ax=ax, cmap=cmap, legend=False)  
...
#Peomean_range_all_years = [min_people_Peomean_all_years + (max_people_Peomean_all_years - min_people_Peomean_all_years) * i / num_bins for i in range(num_bins + 1)]
gdf['bin_Pop'] = pd.cut(gdf[Peomean_column_name], bins=Peomean_range_all_years, labels=range(num_bins), duplicates='drop')
for i, (shp_file, gdf) in enumerate(zip(shp_files, gdf_list)):
    year = re.search(r'\d+', shp_file).group()  # 从文件名中提取数字部分
    title = f"Year:{year}"  # 包含提取年份的标题
    fig, ax = plt.subplots(1, 1, figsize=(40, 40))
