...
output_folder = os.path.join(os.getcwd(), "water街道图中转")
os.makedirs(output_folder, exist_ok=True)

for year in years:
    csv_file_path = f"water-consumption-people{year}year.csv"
    ...
    if os.path.isfile(csv_file_path):
        ...
        output_shapefile = os.path.join(output_folder, f"water-consumption-people {year} year.shp")
...
input_folder = "water街道图中转"
output_folder = "water街道图中转/第一步图像相交结果"
...
for filename in os.listdir(input_folder):
    if filename.endswith(".shp"):
        ...
        result = joined_data.groupby('index_right')['year-consu'].mean().reset_index()
        ...
        missing_faces = gpd.GeoDataFrame(
            {'index_right': list(missing_face_ids), 'year-consu': [0] * len(missing_face_ids)},
...
shp_folder = "water街道图中转\第一步图像相交结果"
gdf_list = []
