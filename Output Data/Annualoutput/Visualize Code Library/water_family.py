...
folder_name = "water family result"
...
output_folder = os.path.join(os.getcwd(), "waterfamily街道图中转")
os.makedirs(output_folder, exist_ok=True)

for year in years:
    csv_file_path = f"water-consumption-family{year}year.csv"
    ...
    if os.path.isfile(csv_file_path):
        ...
        output_shapefile = os.path.join(output_folder, f"water-consumption-family {year} year.shp")
...
input_folder = "waterfamily街道图中转"
output_folder = "waterfamily街道图中转/第一步图像相交结果"
...
for filename in os.listdir(input_folder):
    if filename.endswith(".shp"):
        ...
        result = joined_data.groupby('index_right')['family-w_3'].mean().reset_index()
        ...
        missing_faces = gpd.GeoDataFrame(
            {'index_right': list(missing_face_ids), 'family-w_3': [0] * len(missing_face_ids)},
...
shp_folder = "waterfamily街道图中转\第一步图像相交结果"
gdf_list = []
...
for i, (shp_file, gdf) in enumerate(zip(shp_files, gdf_list)):
    ...
    # 显示图例
    ax.legend(title="Water Family Consumption\nUnit: ton", loc='upper right', fontsize=35)
...
# 等值划分
num_bins = num_bins
price_range_all_years = [min_price_all_years + (max_price_all_years - min_price_all_years) * i / num_bins for i in
                         range(num_bins + 1)]
gdf['bin_residences'] = pd.cut(gdf[price_column_name], bins=price_range_all_years, labels=range(num_bins),
                               duplicates='drop')
for i, (shp_file, gdf) in enumerate(zip(shp_files, gdf_list)):
    year = re.search(r'\d+', shp_file).group()  # 从文件名中提取数字部分
    title = f"Year:{year}"  # 包含提取年份的标题
    ...
    for j in range(num_bins):
        ax.fill_between([], [], color=cmap(j),
                        label=f'{int(price_range_all_years[j])}-{int(price_range_all_years[j + 1])}')
