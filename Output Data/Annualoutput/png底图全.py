...
# 替代的 PNG 底图路径
base_map_path = "basemap/栅格底图.png"
base_map = plt.imread(base_map_path)

base_map_extent = [113.736934, 114.648674, 22.390215, 22.885549]  # 根据底图坐标范围设置
...
for year in years:
    ...
    if coordinates:
        ...
        scaled_house_prices = np.array(house_prices) / 800  # 缩小1000倍
...
for year in years:
    ...
    if coordinates:
        def generate_density_map(coords, base_map):
            ...
            x_grid, y_grid = np.mgrid[113.736934:114.648674:500j, 22.390215:22.885549:500j]  # 使用固定的底图范围
            density = kde.pdf(np.column_stack([x_grid.ravel(), y_grid.ravel()]))
            density = density.reshape(x_grid.shape)
    
            plt.figure(figsize=(92, 50))
            plt.imshow(base_map, extent=[113.736934, 114.648674, 22.390215, 22.885549], aspect='auto')  # 使用固定的底图范围
            ...
            colors[:int(cmap.N / 3):, 3] = 0  # 将后半段颜色的透明度设为0，使其变为透明
    
            modified_cmap = LinearSegmentedColormap.from_list('Modified_RdYlGn_r', colors)
    
            plt.imshow(density.T, origin='lower', extent=[113.736934, 114.648674, 22.390215, 22.885549],
        ...
        def generate_heatmap(coords, base_map):
            ...
            plt.figure(figsize=(92, 50))
            base_map_extent = [113.736934, 114.648674, 22.390215, 22.885549]  # 新的底图范围
